...
def crop_and_scale_image(image, target_size=(512, 512)):
    ...
    image = tf.image.resize(
        image, target_size
    )  # resize - this changes the type to float32
...
normalize_m1to1 = tf.keras.layers.Rescaling(
    scale=1.0 / 127.5, offset=-1
)  # can also use simply: images = images / 127.5 - 1
...
def vis_without_label(M, image, index=None, save=False, dir=None, num_class=26):
    ...
    if save:
        fig.savefig(
            open(dir + "/example_{}.png".format(index), "wb"),
            format="png",
            bbox_inches="tight",
            dpi=200,
        )
